...
def show_cache_info():
    ...
    print(f"Expiry Hours: {info['expiry_hours']} ({'Never' if info['expiry_hours'] <= 0 else 'hours'})")
    print(f"Total Files: {info['total_files']}")
    print(f"Total Size: {info['total_size_bytes']} bytes ({info['total_size_bytes'] / 1024:.1f} KB)")
...
def test_cache_performance():
    ...
    try:
        ...
        print(f"   ✓ Loaded {len(actions)} robot actions and {len(details)} action details")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Robot Action Planner Cache Management")
    ...
    clear_parser.add_argument('--memory', action='store_true', help='Clear in-memory cache only')
