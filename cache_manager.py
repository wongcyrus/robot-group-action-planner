...
from constant import USE_FILE_CACHE, CACHE_DIRECTORY, CACHE_EXPIRY_HOURS
...
class FileCacheManager:
    ...
    def __init__(self):
        ...
        self.cache_dir = os.path.join(os.path.dirname(__file__), CACHE_DIRECTORY)
        self.enabled = USE_FILE_CACHE
        self.expiry_hours = CACHE_EXPIRY_HOURS
        
        if self.enabled:
            self._ensure_cache_directory()
            self.logger.info(f"File cache enabled. Directory: {self.cache_dir}")
    ...
    def cleanup_expired_cache(self) -> int:
        ...
        """
        if not self.enabled or self.expiry_hours <= 0:
            return 0
