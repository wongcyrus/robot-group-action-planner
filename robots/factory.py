...
from actions.robot_action import RobotAction
    ...
    def create_all_robots(
        ...
        try:
            ...
            if self.config.robots.enabled:
                robots["robots"] = self._create_http_robots(
    ...
    def _create_http_robots(
        ...
        try:
            for i, ip in enumerate(self.config.robots.ips):
                ...
                robots.append(robot)
                self.logger.info(f"Created HTTP robot: {robot_id} at {api_url}")
